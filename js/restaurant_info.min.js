let restaurant;var map;window.addEventListener('load',function(){function updateOnlineStatus(event){if(navigator.onLine&&localStorage.getItem('hasOfflineReview')){DBHelper.dbGet((error,reviews)=>{if(error){callback(error,null)}else{if(reviews){reviews.forEach(review=>{let xhr=new XMLHttpRequest();const data=JSON.parse(review.reviews)
    ...
    window.addEventListener('online',updateOnlineStatus)});window.initMap=()=>{fetchRestaurantFromURL((error,restaurant)=>{if(error){console.error(error)}else{self.map=new google.maps.Map(document.getElementById('map'),{zoom:16,center:restaurant.latlng,scrollwheel:!1});fillBreadcrumb();DBHelper.mapMarkerForRestaurant(self.restaurant,self.map)}})}
